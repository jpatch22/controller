...
class Driver:
    def __init__(self):
        ...
        self.left_basic_forward_velocity = 0.25
        self.right_basic_forward_velocity = 0.40
        self.left_turning_constant = 0.006
        self.right_turning_constant = 0.008 #0.06 seems to be perfect works with no derivative
        self.right_turn_k_deriv = 0.0004 * 2 #4 WORKS WELL!
        self.left_turn_k_deriv = 0.0004 * 4

        self.left_previous_forward_velocity = 0.4
    ...
    def follow_right_line(self, main_intercept, turning_intercept):
        ...
        # if abs(self.previous_forward_velocity) - abs(forward_velocity) > 0.2:
        #     print('IN')
        #     forward_velocity = self.previous_forward_velocity + 0.2 * np.sign(self.previous_forward_velocity - forward_velocity)
    ...
    def follow_left_line(self, main_intercept, turning_intercept):
        ...
        if forward_velocity - self.left_forward_speed_reduction_constant * abs(angular_velocty) < 0:
            forward_velocity = 0
        else:
            forward_velocity = forward_velocity - self.left_forward_speed_reduction_constant * abs(angular_velocty)
        if self.previous_forward_velocity - forward_velocity > 0.2:
